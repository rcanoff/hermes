...
# Companion Apple MCP: Hermes injects the active conversation user on each
# HTTP request to the configured ``apple`` MCP server (see api_server patch
# and messaging-api X-Companion-User-Id header).
_APPLE_MCP_SERVER_NAME = "apple"
_COMPANION_USER_HEADER = "X-Companion-User-Id"


def _companion_user_id_for_apple_mcp(server_name: Optional[str] = None) -> Optional[str]:
    """Return companion user UUID for an in-flight apple MCP tool call."""
    try:
    ...
    if user_id:
        return user_id
    if server_name == _APPLE_MCP_SERVER_NAME:
        with _lock:
            server = _servers.get(server_name)
        if server is not None:
            pending = getattr(server, "_active_companion_user_id", None)
            if pending:
                return str(pending).strip() or None
    return None


def _make_companion_header_request_hook(server_name: str):
    """Build an httpx request hook that injects X-Companion-User-Id for apple."""

    async def _inject_companion_user_header(request):
        if server_name != _APPLE_MCP_SERVER_NAME:
            return
        user_id = _companion_user_id_for_apple_mcp(server_name)
        if user_id:
            request.headers[_COMPANION_USER_HEADER] = user_id

    return _inject_companion_user_header
...
class MCPServerTask:
    ...
    __slots__ = (
        ...
        "_pending_call_context", "_active_companion_user_id",
    ...
    def __init__(self, name: str):
        ...
        self._active_companion_user_id: Optional[str] = None
        now = time.monotonic()
    ...
    async def _run_http(self, config: dict):
        ...
        if config.get("transport") == "sse":
            ...
            if True:
                ...
                # TLS plus companion user headers through an httpx_client_factory
                # that wraps the SDK's defaults (follow_redirects=True).
                ...
                _companion_request_hook = _make_companion_header_request_hook(self.name)
                ...
                def _mcp_http_client_factory(
                    ...
                    kwargs: dict = {
                        ...
                        "event_hooks": {"request": [_companion_request_hook]},
                    }
        ...
        if _MCP_NEW_HTTP:
            ...
            client_kwargs: dict = {
                ...
                "verify": ssl_verify,
                "event_hooks": {
                    "request": [_companion_request_hook],
...
def _snapshot_session_context() -> Optional[dict]:
    """Capture gateway session context from the scheduling thread.

    MCP tool handlers run on a background event loop via
    ``run_coroutine_threadsafe``, which does not inherit the agent
    thread's ContextVars. Companion Apple MCP needs
    ``HERMES_SESSION_USER_ID`` on that loop so the HTTP request hook can
    inject ``X-Companion-User-Id``.
    """
    try:
...
def _wrap_with_session_context(coro: "Coroutine") -> "Coroutine":
    """Carry gateway session ContextVars into a coroutine on the MCP loop."""
    snapshot = _snapshot_session_context()
    if not snapshot:
        return coro
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Same pattern for companion session vars (e.g. HERMES_SESSION_USER_ID
    # for apple MCP X-Companion-User-Id injection on api_server turns).
    coro = _wrap_with_session_context(coro)
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        companion_user_id = (
            _companion_user_id_for_apple_mcp(server_name)
            if server_name == _APPLE_MCP_SERVER_NAME
            else None
        )
        ...
        async def _call():
            ...
            async with server._rpc_lock, _track_inflight_rpc(
                ...
                server._active_companion_user_id = companion_user_id
                try:
                    ...
                    server._active_companion_user_id = None
