...
def get_profile_dir(name: str) -> Path:
    ...
    if "/" in canon:
        ...
        if len(parts) != 2 or not parts[0] or not parts[1]:
            raise ValueError(
                f"Invalid profile name {canon!r}. Namespaced profiles must be "
                f"<namespace>/<slug>."
            )
        if not _PROFILE_ID_RE.match(parts[0]) or not _PROFILE_ID_RE.match(parts[1]):
            raise ValueError(
                f"Invalid profile name {canon!r}. Must match "
                f"[a-z0-9][a-z0-9_-]{{0,63}}(/[a-z0-9][a-z0-9_-]{{0,63}})?"
            )
        return _get_profiles_root() / parts[0] / parts[1]
...
def _is_live_profile_id_dir(entry: Path, *, allow_default: bool = False, live_only: bool = True) -> bool:
    """True when *entry* is a non-deleted ``profiles/<id>/`` candidate."""
    if not entry.is_dir():
        return False
    name = entry.name
    ...
    if not _PROFILE_ID_RE.match(name):
        return False
    return not (live_only and named_profile_is_deleted(entry))
...
def _iter_named_profile_dirs(*, live_only: bool = True) -> List[Path]:
    ...
    return [
        ...
        if _is_live_profile_id_dir(entry, live_only=live_only)
    ]
