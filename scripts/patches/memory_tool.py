...
_DEFAULT_MEMORY_USERNAME = "rcanoff"


def get_memory_dir() -> Path:
    """Profile-scoped memories dir, resolved per call (HERMES_HOME may switch after import).

    Companion turns bind ``HERMES_SESSION_USER_NAME`` (from ``X-Companion-Username``).
    File backup then lives under ``memories/users/<username>/``. Unset (TUI/CLI) uses
    the local Companion user ``rcanoff``, not the old root USER.md/MEMORY.md pair.
    """
    username = ""
    try:
        from gateway.session_context import get_session_env

        username = (get_session_env("HERMES_SESSION_USER_NAME", "") or "").strip()
    except Exception:
        username = ""
    if not username:
        username = (
            os.environ.get("HERMES_SESSION_USER_NAME")
            or os.environ.get("HERMES_SESSION_USERNAME")
            or ""
        ).strip()
    if not username:
        username = _DEFAULT_MEMORY_USERNAME
    safe = re.sub(r"[^A-Za-z0-9_-]", "-", username).strip("-") or _DEFAULT_MEMORY_USERNAME
    return get_hermes_home() / "memories" / "users" / safe
