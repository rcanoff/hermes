...
def get_memory_dir() -> Path:
    ...
    Companion turns bind ``HERMES_SESSION_USER_NAME`` (from
    ``X-Companion-Username``). File backup then lives under
    ``memories/users/<username>/``. Unset (TUI/CLI) uses the local
    Companion user ``rcanoff``, not the old root USER.md/MEMORY.md pair.
    """
    username = ""
    try:
